...
class Encoding_matrix:
    ...
    def pkbse_encoding_matrix( self ):
        ...
        Note: We are thinking of the subdivisions in terms on if we were improvising. We make musical decisions based on
...
class QuikoCircuit: ## This needs a new name for better description
    ...
    def amplitude_phase_encoding(self, add_measurement=False):
        """
        Encodes the database audio samples without a spinal cord register.
        If `add_measurement` is True, adds measurement gates at the end.
        
        Note: For initial analysis, no internal pulse is added on the timbre qubits
            (i.e., no Hadamard gates are applied).
        """
        print("Encoding parameters:")

        for band, params in enumerate(self.encoding):
            # Each band is encoded with a u gate using parameters for amplitude and phase
            print(f"Encoding qubit {band} with parameters {params}")
            self.qc.u(params[0], params[1], params[2], band)
        
        # Add measurement gates if specified
        if add_measurement:
            self.qc.measure_all()
        ...
        for subdiv in range( 2**self.spine_qubits ):
                ...
                for band, seq in enumerate( self.encoding ):    
                    for subdiv, param in enumerate( seq ):
                        ...
                        qc_u.u( param[0], param[1], param[2], 0 )
    ...
    def pkbse_encoding( self, num_layers=1):
        """
                ...
                for band, band_elements in enumerate(self.encoding):
                    for subdivi in range(int((2 ** self.spine_qubits) / 2)):
                        ...
                        self.qc.cu(
                            ...
                            lam=gate_params[2],
                            gamma=0,
                            control_qubit=total_qubits - (self.spine_qubits - band),
                            target_qubit=band,
                            label=None,
                            ctrl_state=None
                        )
        ...
        # Optional: Add additional operations or separation between layers, if needed
        # Example: You can include barrier or marker gates for visualization
        self.qc.barrier()
              
    def Quantum_Circuit( self, Encoding_method, n_layers ):
        """
        ...
        if Encoding_method == 0:
            ...
            self.amplitude_phase_encoding(add_measurement = False)
            
        elif Encoding_method == 1:
            self.internal_pulse( static_timbre=False )
            self.pkbse_encoding(n_layers)
