...
def p_proc( subbands, fs ): #(dl, dm, dh, fs):
    """
    Processes subbands to extract harmonic and percussive components for database initialization.

    Parameters:
    - subbands (list): A list of subbands, where each subband is a 1D NumPy array representing a portion of the frequency spectrum.
    - fs (int): Sampling frequency of the audio data (e.g., 44100 Hz).

    Returns:
    - sources (dict): A dictionary containing the processed harmonic and percussive components for each subband:
        - Key: Band identifier as a string (e.g., "Band_0", "Band_1", ...).
        - Value: A list containing:
            - `iyh` (ndarray): Reconstructed harmonic component for the subband in the time domain.
            - `iyp` (ndarray): Reconstructed percussive component for the subband in the time domain.
    """
    sources = {}
...
def onset( data, fs ): ## The full audio sample here
    max_amp = np.max( data )
    o_env = librosa.onset.onset_strength(y=data, sr=fs)
